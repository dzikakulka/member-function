...
;; Thu Jun 04 10:43:10 2015 Added handling for multiple characters in header
;; file extension - now working with e.g. ".hpp" instead of only ".h".
;; * Edited (mf--infer-c-filename) function.
;; Inform about better solution at github.com/dzikakulka/member-functions
;; dzikakulka@gmail.com 
...
(defun mf--infer-c-filename (header-filename)
  ...
  (if (string-match 
       ...
       header-filename)
	  (concat (substring header-filename 0 (-(length mf--header-file-extension))) mf--source-file-extension)
    nil))
